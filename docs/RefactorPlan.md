...
- Choose codegen handling strategy:
  - Option A (minimal generator change): run Invoke-CodeGen.ps1 as today, then move generated files into per-client folders via post-step; csproj includes moved files.
  - Option B (preferred if feasible): update generator templates/targets to emit into per-client folders/namespaces directly, minimizing move logic.
...
- Inspect Invoke-CodeGen.ps1 and codegen/emitter outputs; spike Option A vs Option B.
- If Option B: adjust generator templates and file output paths per client namespace; ensure partial classes remain co-located when needed.
- If Option A: add structured move step after generation; maintain mappings from namespace → target project folder; validate incremental builds.
- Update TSP specs or emitter configs to emit client-specific namespaces matching package IDs.
- Add CI check to ensure no generated files remain in monolithic locations after move (e.g., diff check on src/Generated).
...
- Move to OpenAI.Shared with namespace updates; expose clear public surface for reuse.
...
- For each client area (Chat, Responses, Assistants, Audio, Images, Embeddings, Moderations, Models, Batch, Realtime, etc.):
  - Move generated + handwritten code into the package folder.
  - Update namespaces to package-specific roots.
  - Ensure csproj includes only owned files (wildcards or explicit includes post-move).
...
- [ ] Adjust codegen for Responses: emit under OpenAI.Responses namespace and Responses folder, or add post-generation move step mapping generated files to src/Responses; keep deterministic output.
- [ ] Move handwritten Responses client code (client class, helpers, streaming support if any) into src/Responses; update namespaces.
