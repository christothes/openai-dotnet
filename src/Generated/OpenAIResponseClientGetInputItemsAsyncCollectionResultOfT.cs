// <auto-generated/>

#nullable disable

using System;
using System.ClientModel;
using System.ClientModel.Primitives;
using System.Collections.Generic;
using System.Threading.Tasks;
using OpenAI;

namespace OpenAI.Responses
{
    internal partial class OpenAIResponseClientGetInputItemsAsyncCollectionResultOfT : AsyncCollectionResult<ResponseItem>
    {
        private readonly OpenAIResponseClient _client;
        private readonly string _responseId;
        private readonly long? _limit;
        private readonly string _order;
        private readonly string _after;
        private readonly string _before;
        private readonly RequestOptions _options;

        public OpenAIResponseClientGetInputItemsAsyncCollectionResultOfT(OpenAIResponseClient client, string responseId, long? limit, string order, string after, string before, RequestOptions options)
        {
            Argument.AssertNotNullOrEmpty(responseId, nameof(responseId));

            _client = client;
            _responseId = responseId;
            _limit = limit;
            _order = order;
            _after = after;
            _before = before;
            _options = options;
        }

        public override async IAsyncEnumerable<ClientResult> GetRawPagesAsync()
        {
            PipelineMessage message = _client.CreateGetInputItemsRequest(_responseId, _limit, _order, _after, _before, _options);
            string nextToken = null;
            while (true)
            {
                ClientResult result = ClientResult.FromResponse(await _client.Pipeline.ProcessMessageAsync(message, _options).ConfigureAwait(false));
                yield return result;

                // Plugin customization: add hasMore assignment
                bool hasMore = ((InternalResponseItemList)result).HasMore;
                nextToken = ((InternalResponseItemList)result).LastId;
                // Plugin customization: add hasMore == false check to pagination condition
                if (nextToken == null || !hasMore)
                {
                    yield break;
                }
                message = _client.CreateGetInputItemsRequest(_responseId, _limit, _order, nextToken, _before, _options);
            }
        }

        public override ContinuationToken GetContinuationToken(ClientResult page)
        {
            string nextPage = ((InternalResponseItemList)page).LastId;
            if (nextPage != null)
            {
                return ContinuationToken.FromBytes(BinaryData.FromString(nextPage));
            }
            else
            {
                return null;
            }
        }

        protected override async IAsyncEnumerable<ResponseItem> GetValuesFromPageAsync(ClientResult page)
        {
            foreach (ResponseItem item in ((InternalResponseItemList)page).Data)
            {
                yield return item;
                await Task.Yield();
            }
        }
    }
}
