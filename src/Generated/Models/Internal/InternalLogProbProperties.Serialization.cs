...
{
    internal partial class InternalLogProbProperties : IJsonModel<InternalLogProbProperties>
    {
        internal InternalLogProbProperties() : this(null, default, null, null)
        {
        }
        ...
        InternalLogProbProperties IJsonModel<InternalLogProbProperties>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options) => JsonModelCreateCore(ref reader, options);

        protected virtual InternalLogProbProperties JsonModelCreateCore(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<InternalLogProbProperties>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(InternalLogProbProperties)} does not support reading '{format}' format.");
            }
            using JsonDocument document = JsonDocument.ParseValue(ref reader);
            return DeserializeInternalLogProbProperties(document.RootElement, options);
        }

        internal static InternalLogProbProperties DeserializeInternalLogProbProperties(JsonElement element, ModelReaderWriterOptions options)
        {
