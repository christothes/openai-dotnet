...
namespace OpenAI.Realtime
{
    internal partial class InternalVadConfig : IJsonModel<InternalVadConfig>
    {
        ...
        protected virtual void JsonModelWriteCore(Utf8JsonWriter writer, ModelReaderWriterOptions options)
        {
            ...
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(InternalVadConfig)} does not support writing '{format}' format.");
            }
        ...
        InternalVadConfig IJsonModel<InternalVadConfig>.Create(ref Utf8JsonReader reader, ModelReaderWriterOptions options) => JsonModelCreateCore(ref reader, options);

        protected virtual InternalVadConfig JsonModelCreateCore(ref Utf8JsonReader reader, ModelReaderWriterOptions options)
        {
            string format = options.Format == "W" ? ((IPersistableModel<InternalVadConfig>)this).GetFormatFromOptions(options) : options.Format;
            if (format != "J")
            {
                throw new FormatException($"The model {nameof(InternalVadConfig)} does not support reading '{format}' format.");
            }
            using JsonDocument document = JsonDocument.ParseValue(ref reader);
            return DeserializeInternalVadConfig(document.RootElement, options);
        }

        internal static InternalVadConfig DeserializeInternalVadConfig(JsonElement element, ModelReaderWriterOptions options)
        {
